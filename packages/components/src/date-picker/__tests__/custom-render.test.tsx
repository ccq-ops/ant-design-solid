...
describe('DatePicker custom rendering and visual APIs', () => {
  it('keeps selected and range endpoint text visible on hover and active states', () => {
    ...
    expect(css).toContain(
      '.ads-date-picker-grid{display:grid;gap:4px;grid-template-columns:repeat(7, 1fr);',
    )
    expect(css).toContain('.ads-date-picker-grid .ads-date-picker-cell{width:100%;')
    expect(css).toContain(
      '.ads-date-picker-weekdays{display:grid;gap:4px;grid-template-columns:repeat(7, 1fr);',
    )
  ...
  it('aligns week and variant picker cell styles with antd panels', () => {
    ...
    expect(css).toContain(
      '.ads-date-picker-variant-grid.ads-date-picker-quarter-grid{grid-template-columns:repeat(4, 1fr);',
    )
    expect(css).toContain(
      '.ads-date-picker-week-row:not(.ads-date-picker-week-row-selected):hover{background:rgba(0,0,0,0.04);',
    )
    ...
    expect(css).toContain(
      '.ads-date-picker-week-row .ads-date-picker-week-cell{align-items:center;display:inline-flex;justify-content:center;width:100%;',
    )
    ...
    expect(css).toContain(
      '.ads-date-picker-week-cell:not(.ads-date-picker-cell-selected):not(.ads-date-picker-cell-disabled){color:rgba(0,0,0,0.88);',
    )
  })
