...
**Goal:** Add a tested, documented `Menu` component to `@solid-ant-design/core` with item, group, divider, and submenu support.
...
**Tech Stack:** SolidJS, TypeScript, Vitest, `@solidjs/testing-library`, `@solid-ant-design/cssinjs`, repository config-provider and class-name utilities.
...
```bash
COREPACK_ENABLE_DOWNLOAD_PROMPT=0 corepack pnpm --filter @solid-ant-design/core test -- menu
```
