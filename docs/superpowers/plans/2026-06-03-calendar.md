...
**Goal:** Add a tested, documented Ant Design-style `Calendar` component to `@ant-design-solid/core`.
...
**Tech Stack:** SolidJS components/signals/memos, `@solidjs/testing-library`, Vitest, CSS-in-JS via `@ant-design-solid/cssinjs`, workspace docs app routes.
...
describe('Calendar', () => {
  ...
  it('supports controlled mode', () => {
    function Demo() {
      ...
      return <Calendar defaultValue="2026-06-15" mode={mode()} onPanelChange={(_, next) => setMode(next)} />
    }
  ...
  it('supports full cell render overrides', () => {
    render(() => (
      <Calendar
        defaultValue="2026-06-15"
        dateFullCellRender={(date) => (date.getDate() === 15 ? <strong>Full date</strong> : date.getDate())}
        monthFullCellRender={(date) =>
          date.getMonth() === 5 ? <strong>Full month</strong> : date.toLocaleString('en-US', { month: 'short' })
        }
      />
    ))
...
```bash
COREPACK_ENABLE_DOWNLOAD_PROMPT=0 corepack pnpm --filter @ant-design-solid/core test -- calendar
```
...
export function Calendar(props: CalendarProps) {
  ...
  function renderDefaultHeader() {
    return (
      <div class={`${prefixCls()}-header`}>
        <div class={`${prefixCls()}-title`}>{mode() === 'month' ? formatMonth(panelDate()) : panelDate().getFullYear()}</div>
        <div class={`${prefixCls()}-header-actions`}>
          <button type="button" aria-label="Previous year" class={`${prefixCls()}-button`} onClick={() => changePanelDate(addYears(panelDate(), -1))}>
            «
          </button>
          <Show when={mode() === 'month'}>
            <button type="button" aria-label="Previous month" class={`${prefixCls()}-button`} onClick={() => changePanelDate(addMonths(panelDate(), -1))}>
              ‹
            </button>
          </Show>
          <Show when={mode() === 'month'}>
            <button type="button" aria-label="Next month" class={`${prefixCls()}-button`} onClick={() => changePanelDate(addMonths(panelDate(), 1))}>
              ›
            </button>
          </Show>
          <button type="button" aria-label="Next year" class={`${prefixCls()}-button`} onClick={() => changePanelDate(addYears(panelDate(), 1))}>
            »
          </button>
          <button
            type="button"
            ...
            class={classNames(`${prefixCls()}-button`, mode() === 'month' && `${prefixCls()}-button-active`)}
          ...
          <button
            ...
            class={classNames(`${prefixCls()}-button`, mode() === 'year' && `${prefixCls()}-button-active`)}
  ...
  return (
    ...
    >
      <Show
        when={local.headerRender}
        fallback={renderDefaultHeader()}
      >
        {(headerRender) =>
          headerRender()({
            value: panelDate(),
        ...
        <Show when={mode() === 'month'} fallback={
                ...
                return (
                  ...
                  >
                    <Show
                      ...
                      fallback={
                        <>
                          <span class={`${prefixCls()}-date-value`}>{MONTHS[monthDate.getMonth()]}</span>
                          <Show when={local.monthCellRender?.(monthDate)}>
                            {(content) => <div class={`${prefixCls()}-cell-content`}>{content()}</div>}
                          </Show>
                        </>
                      }
                    >
          ...
          <div>
            <div class={`${prefixCls()}-weekdays`}>
              <For each={WEEKDAYS}>{(weekday) => <div class={`${prefixCls()}-weekday`}>{weekday}</div>}</For>
            </div>
...
/>`}
      >
        <Calendar
          ...
          dateCellRender={(date) => (sameDay(date, new Date(2026, 5, 15)) ? <span>Selected</span> : null)}
        />
      </DemoBlock>
      ...
      <ul>
        ...
        <li>
          Use <code>dateCellRender</code> or <code>dateFullCellRender</code> to customize date cells.
        </li>
        <li>
          Use <code>monthCellRender</code> or <code>monthFullCellRender</code> to customize year panel month cells.
        </li>
      </ul>
    </>
  )
...
```bash
COREPACK_ENABLE_DOWNLOAD_PROMPT=0 corepack pnpm --filter @ant-design-solid/docs test -- routes
```
