import { Badge, Button, Card, Space, Tag } from '@ant-design-solid/core'
import { useNavigate } from '@solidjs/router'
import heroBanner from '../assets/hero-banner.png'

export const $$SolidBase_page_data = {
  frontmatter: {},
  toc: undefined,
  editLink: '',
  lastUpdated: 0,
}

export const features = [
  {
    title: 'Component-rich',
    description:
      'Explore Ant Design-inspired semantics across navigation, data entry, feedback, and display components.',
  },
  {
    title: 'Token-driven theming',
    description:
      'Tune the system from ConfigProvider with seed tokens, component tokens, and predictable visual results.',
  },
  {
    title: 'Solid-native runtime',
    description:
      'Use fine-grained reactivity with a Solid-first component model and CSS-in-JS runtime built for this stack.',
  },
]

export default function Home() {
  const navigate = useNavigate()

  return (
    <div class="space-y-10 pb-8">
      <section class="docs-hero docs-border relative overflow-hidden rounded-[36px] border px-6 py-14 text-center sm:px-10 lg:px-14">
        <div class="docs-hero-overlay pointer-events-none absolute inset-0" />
        <div class="pointer-events-none absolute left-1/2 top-28 h-64 w-[min(760px,86%)] -translate-x-1/2 rounded-full bg-blue-300/20 blur-3xl" />

        <div class="relative z-[1] mx-auto max-w-[860px]">
          <div class="docs-border-subtle docs-surface-muted mb-6 inline-flex rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-blue-600 shadow-sm backdrop-blur">
            SolidJS · Design System · Token Driven
          </div>
          <h1 class="docs-text-strong mb-5 mt-0 text-5xl font-black tracking-[-0.045em] sm:text-6xl lg:text-7xl">
            Ant Design Solid
          </h1>
          <p class="docs-text-strong mx-auto mb-2 max-w-[720px] text-2xl font-semibold tracking-[-0.025em] sm:text-3xl">
            Ant Design for the Solid era
          </p>
          <p class="docs-text-secondary mx-auto max-w-[760px] text-base leading-[1.8] sm:text-lg">
            Build polished product interfaces with Ant Design-inspired semantics, Solid-native
            performance, token-driven theming, and a docs experience designed for fast discovery.
          </p>
          <Space class="mt-8 justify-center" size="middle">
            <Button type="primary" size="large" onClick={() => navigate('/docs/getting-started')}>
              Get Started
            </Button>
            <Button size="large" onClick={() => navigate('/components/affix')}>
              View Components
            </Button>
          </Space>
        </div>

        <div class="relative z-[1] mx-auto mt-12 max-w-[960px]">
          <div class="docs-border docs-surface-glass docs-card-shadow rounded-[32px] border p-2 backdrop-blur-xl">
            <img
              alt="Blue glassmorphism interface hero artwork"
              class="w-full rounded-[26px] object-cover"
              src={heroBanner}
            />
          </div>
        </div>
      </section>

      <section class="grid gap-5 lg:grid-cols-3">
        {features.map((feature) => (
          <Card class="docs-border-subtle docs-card-shadow h-full">
            <div class="docs-primary-soft mb-4 flex h-11 w-11 items-center justify-center rounded-2xl text-lg text-blue-600">
              ✦
            </div>
            <h2 class="docs-text-strong mb-3 mt-0 text-xl font-bold">{feature.title}</h2>
            <p class="docs-text-secondary m-0 leading-[1.75]">{feature.description}</p>
          </Card>
        ))}
      </section>

      <section class="docs-border-subtle docs-surface-solid docs-float-shadow grid gap-6 rounded-[32px] border p-6 lg:grid-cols-[1fr_1.25fr] lg:p-8">
        <div>
          <Tag color="blue">Components</Tag>
          <h2 class="docs-text-strong mb-3 mt-4 text-3xl font-black tracking-[-0.035em]">
            Preview the building blocks
          </h2>
          <p class="docs-text-secondary mb-6 leading-[1.75]">
            Ready for production docs, examples, and theme exploration without leaving the SolidJS
            workflow.
          </p>
          <Space wrap>
            <Badge count={32}>
              <Button>Components</Button>
            </Badge>
            <Button type="primary">Primary action</Button>
            <Button>Secondary</Button>
          </Space>
        </div>

        <Card class="docs-border docs-surface-subtle">
          <div class="mb-4 flex gap-2 text-sm font-medium">
            <span class="rounded-lg bg-blue-50 px-3 py-1.5 text-blue-600">Theme</span>
            <span class="rounded-lg px-3 py-1.5 text-slate-500">Runtime</span>
          </div>
          <div class="docs-surface-solid docs-card-shadow rounded-2xl p-5">
            <div class="docs-text-strong mb-3 text-sm font-semibold">Token preview</div>
            <div class="flex gap-3">
              <span class="h-10 flex-1 rounded-xl bg-blue-500" />
              <span class="h-10 flex-1 rounded-xl bg-indigo-500" />
              <span class="h-10 flex-1 rounded-xl bg-violet-500" />
            </div>
          </div>
          <div class="docs-surface-solid docs-text-secondary docs-card-shadow mt-4 rounded-2xl p-5 text-sm leading-7">
            Fine-grained updates, component-level styles, and predictable composition for Solid
            applications.
          </div>
        </Card>
      </section>
    </div>
  )
}
